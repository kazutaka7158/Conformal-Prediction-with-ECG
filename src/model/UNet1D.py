...
from src.utils.loss import FocalLoss, dice_score
...
class ECGUNet3pCGMNet(nn.Module):
    def __init__(self, n_channels=4, mask=True):
        ...
        self.down3 = StackEncoder(filters[1], filters[2])
        self.down4 = StackEncoder(filters[2], filters[3])
        self.middle = nn.Sequential(ConvBnRelu1d(filters[3], filters[4]),
...
class ECGUNet3pCGM(L.LightningModule):
    def __init__(self, lr=1e-3, focal_gamma=2.0):
        super().__init__()
        self.save_hyperparameters()
        self.model = ECGUNet3pCGMNet()
        self.lr = lr
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.criterion(seg_pred, seg_target)
        dice_score = self.__dice_score(seg_pred, seg_target)
        self.log("train_dice", dice_score.mean(), on_step=False, on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_dice", dice_score.mean(), on_step=False, on_epoch=True, prog_bar=True)
        return loss
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("test_dice", dice_score.mean(), on_step=False, on_epoch=True, prog_bar=True)
        return loss
    ...
    def __dice_score(self, seg_pred, seg_target):
        dice = dice_score(seg_pred, seg_target)
        return dice
